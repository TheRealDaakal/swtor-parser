"""HTTP route implementation for the SWTOR Parser local web UI.

The public compatibility API remains in :mod:`web_server`; this module owns
the request handler and its route logic.
"""
...
import log_watcher
import storage
from rotation_analysis import rotation_segments

from web_server_helpers import (
    CorpusState, _safe_encounter_id, build_ability_breakdown, build_history_detail,
    build_history_list, build_live_snapshot,
)
        ...
        def do_POST(self):
            ...
            if u.path == "/api/timer_rules":
                ...
                try:
                    ...
                    warn = 0.0
                try:
                    countdown_from = max(0, int(body.get("countdown_from") or 0))
                except (TypeError, ValueError):
                    countdown_from = 0
                ...
                rule = TimerRule(
                    ...
                    audio_path=audio_path, countdown_from=countdown_from,
                )
